...
int main() {
  ...
  for (int log_num = 1; log_num <= 93; log_num++) {
    ...
    printf("Loaded tracklets for log %d\n", log_num);
    ...
    int scan_id = 0;
    while (true) {
      sprintf(fn, "/home/aushani/data/kittidata/extracted/2011_09_26/2011_09_26_drive_%04d_sync/velodyne_points/data/%010d.bin", log_num, scan_id);
      if (!FileExists(fn)) {
        break;
      }
      ...
      for (const auto &hit: scans[scan_id].GetHits()) {
    ...
    for (auto it = models.begin(); it != models.end(); it++) {
      ...
      ss << it->first << "_" << log_num << ".csv";
      std::ofstream stats_file(ss.str());
